...
一応。ロールプレイの一環としてキャラクターのロストを認めているシステムが存在する。例えば『トーキョー N◎VA』である。このシステムは「とにかくかっこいいロールプレイをする」という事を主軸にしているため、いよいよとなったらプレイヤーのロストを視野に入れる事がシステムでサポートされている。先に言っておく。 **システムでサポートされているからギリギリ許されている** だけである事には留意していただきたい。

N◎VAのキャラロストにはきちんと条件もついている。「そのキャラクターのロストはキャラクターが最大限に活躍し(あるいはしようとした)結果である事」「セッションの終盤であること」。得に前者についてはその判定は厳しいものでなければならず、ただの自己犠牲や自殺行為では到底認められない。
...
### 明確な成功/失敗以外の大袈裟なマルチエンド、シナリオ分岐 {#multiple-end}
